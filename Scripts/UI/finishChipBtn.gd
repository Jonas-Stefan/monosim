...
func create_new_chip() -> void:
	...
	var oldGateNodes: Array = []
	var gateTypes: Array[String] = []
	
	var mainNodes = SimEngine.mainGraph.GetNodes()
	var mainEdges = SimEngine.mainGraph.GetEdges()
	var chipGraphs = SimEngine.GetChipGraphs()
	...
	for lamp in lamps:
		...
		for edge in lamp.inputs[0].connectedWire.edges:
			var connectedOldNode = edge.input
